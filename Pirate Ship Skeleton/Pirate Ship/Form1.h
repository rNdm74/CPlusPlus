	...
	private: System::Void timer1_Tick(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			if(collided())
			{
				...
				MessageBox::Show("You sunk the ship!!");
			}	

			// Checks if the cannon ball missed			
			if(cannonBallY > ClientRectangle.Bottom ||
			   cannonBallY < ClientRectangle.Top	||
			   cannonBallX > ClientRectangle.Right	||
			   cannonBallX < ClientRectangle.Left)
			{
				timer1->Enabled = false;
...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			int power = Convert::ToInt16(tbPower->Value);

			// Read the degree of rotation from trackbar tbAim and convert to an int
			int angle = Convert::ToInt16(tbAim->Value);	
			...
			velocityX = Math::Cos(radiansAngle) * power;
			velocityY = -(Math::Sin(radiansAngle) * power); // starts at a negative velocity
